...
# Prompt for values
read -s -p "Enter your map's API token: " API_TOKEN
echo  # Add newline after silent input
read -p "Enter your map's slug (from URL): " MAP_SLUG
read -s -p "Enter ACL API token (optional, press Enter to skip): " ACL_API_TOKEN
echo  # Add newline after silent input
...
if [ -z "$MAP_SLUG" ]; then
    echo "Error: Map slug is required"
    exit 1
fi

# Update .env file
if ! cat > test/api/.env << EOF
