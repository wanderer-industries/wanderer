defmodule WandererAppWeb.Integration.Api.V1.MapApiV1Test do
  ...
  describe "GET /api/v1/maps - security and filtering" do
    ...
    test "returns only maps owned by the token's user", %{map1: map1} do
      conn =
        build_conn()
      ...
      # Should see map1 and map2 (both owned by user1)
      map_ids = Enum.map(maps, & &1["id"])
      assert length(map_ids) == 2
      assert map1.id in map_ids
    end

    test "does not return maps owned by other users", %{map1: map1, map3: map3} do
      conn =
        build_conn()
        |> put_req_header("authorization", "Bearer #{map1.public_api_key}")
        |> put_req_header("content-type", "application/vnd.api+json")
      ...
      # Should NOT see map3 (owned by user2)
      map_ids = Enum.map(maps, & &1["id"])
      refute map3.id in map_ids
    end

    test "returns maps shared via ACL", %{map3: map3, character2: character2} do
      ...
      # Find map2 in the results
      map_names = Enum.map(maps, & &1["attributes"]["name"])
      assert "User1 Map 2" in map_names
      assert "User2 Map" in map_names
    end
    ...
    test "does not return deleted maps", %{map1: map1, character1: character1} do
      ...
      # Should not see the deleted map
      map_ids = Enum.map(maps, & &1["id"])
    ...
    test "returns correct map attributes", %{map1: map1} do
      ...
      assert %{"data" => [map_data | _]} = json_response(conn, 200)

      # Verify standard JSON:API structure
      assert map_data["type"] == "maps"
      assert map_data["id"]
      assert map_data["attributes"]["name"]
      assert map_data["attributes"]["slug"]
    end
  ...
  describe "GET /api/v1/maps - ACL filtering edge cases" do
    ...
    test "includes maps shared with user's alternate characters", %{
      map1: map1,
      shared_map: shared_map
    } do
      conn =
      ...
      # Should see both maps: owned map and shared map
      map_ids = Enum.map(maps, & &1["id"])
      assert map1.id in map_ids
      assert shared_map.id in map_ids
    end
  end
  ...
  describe "GET /api/v1/maps - corporation and alliance ACLs" do
    ...
    test "includes maps shared via corporation ACL", %{map: map, corp_shared_map: corp_shared_map} do
      conn =
        build_conn()
      ...
      map_ids = Enum.map(maps, & &1["id"])
      assert corp_shared_map.id in map_ids
    end

    test "includes maps shared via alliance ACL", %{
      map: map,
      alliance_shared_map: alliance_shared_map
    } do
      conn =
      ...
      map_ids = Enum.map(maps, & &1["id"])
      assert alliance_shared_map.id in map_ids
    end
  end
end
