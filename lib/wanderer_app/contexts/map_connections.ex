defmodule WandererApp.Contexts.MapConnections do
  ...
  """
  @spec list_connections_for_system(String.t(), integer()) :: [map()] | {:error, atom()}
  def list_connections_for_system(map_id, system_id) do
    case list_connections(map_id) do
      {:error, _} = error ->
        error

      connections ->
        # Filter connections where system_id is either source or target
        connections
        |> Enum.filter(fn c ->
