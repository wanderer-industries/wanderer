defmodule WandererApp.Api.MapSystem do
  ...
  actions do
    ...
    create :create do
      ...
      # Auto-inject map_id from authenticated token (overrides any client-provided value)
    ...
    update :update do
      ...
      argument :labels, {:array, :string}, allow_nil?: true
        ...
        Enum.reduce(
          ...
          fn field, acc ->
            case Ash.Changeset.get_argument(acc, field) do
              nil -> acc
              value -> Ash.Changeset.change_attribute(acc, field, value)
            end
          end
        )
      end
      ...
      change fn changeset, _context ->
        ...
        case Ash.Changeset.get_attribute(changeset, :labels) do
          ...
          labels when is_binary(labels) ->
            cleaned_labels = WandererApp.MapSystemRepo.clean_labels(labels, map_opts)
