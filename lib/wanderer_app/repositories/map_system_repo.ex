defmodule WandererApp.MapSystemRepo do
  ...
  - **Internal callers**: When called internally (e.g., map duplication, seeds,
    background jobs), this function loads the map to provide context for
    `InjectMapFromActor`.
  ...
  - `{:error, reason}` if map not found or creation fails

  ## Examples

      # Internal creation (loads map)
      ...
      # API creation (map in actor, no additional load)
  ...
  def update_position_and_attributes!(system, attrs, opts \\ []) do
    case update_position_and_attributes(system, attrs, opts) do
      {:ok, updated_system} -> updated_system
      {:error, changeset} -> raise Ash.Error.to_error_class(changeset)
    end
  end
