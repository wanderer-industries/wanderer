defmodule WandererApp.Map.ZkbDataFetcher do
  ...
  defp update_map_kills(map_id) do
    with_started_map(map_id, "basic kills update", fn ->
      ...
      |> Enum.into(%{}, fn {solar_system_id, _system} ->
        ...
        kills_count = WandererApp.Cache.get(CacheKeys.system_kill_count(solar_system_id)) || 0
  ...
  defp update_detailed_map_kills(map_id) do
    with_started_map(map_id, "detailed kills update", fn ->
      ...
      cache_key_ids = CacheKeys.map_killmail_ids(map_id)
      cache_key_details = CacheKeys.map_detailed_kills(map_id)
      ...
      new_ids_map =
        Enum.into(systems, %{}, fn {solar_system_id, _} ->
          ...
          ids = WandererApp.Cache.get(CacheKeys.system_kill_list(solar_system_id)) || []
  ...
  defp maybe_broadcast_map_kills(new_kills_map, map_id) do
    {:ok, old_kills_map} = WandererApp.Cache.lookup(CacheKeys.map_kill_counts(map_id), %{})
    ...
    if changed_system_ids == [] do
      ...
      :ok =
        WandererApp.Cache.insert(CacheKeys.map_kill_counts(map_id), new_kills_map,
