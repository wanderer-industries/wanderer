defmodule WandererApp.ExternalEvents.DiscordDispatcher do
  ...
  defp explain_drop({:drop, :no_system}), do: "the event carries no solar system id"
  ...
  defp extract_kills(%{"type" => :killmail_update} = payload) do
    case {payload["solar_system_id"], payload["killmails"]} do
      # Without the system id there is nothing to filter on and nothing to name:
      # wh_only would drop it as "not wormhole space" (misleading), and with
      # wh_only off it would render an embed for a nil system.
      {nil, _} -> {:drop, :no_system}
      {system_id, [_ | _] = kills} -> {:ok, system_id, kills}
      _ -> :skip
    end
  end
