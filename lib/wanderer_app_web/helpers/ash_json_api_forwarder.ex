defmodule WandererAppWeb.Helpers.AshJsonApiForwarder do
      ...
      body: %{
        "errors" =>
          Enum.map(error.errors, fn e ->
            %{
              "field" => safely_stringify_field(e.field),
  ...
  # Safely convert field to string with proper type handling
  defp safely_stringify_field(field) when is_binary(field), do: field
  defp safely_stringify_field(field) when is_atom(field), do: Atom.to_string(field)
  defp safely_stringify_field(field) when is_integer(field), do: Integer.to_string(field)
  defp safely_stringify_field(field) when is_list(field), do: Enum.join(field, ".")
  defp safely_stringify_field(nil), do: "base"
  defp safely_stringify_field(_field), do: "unknown"
