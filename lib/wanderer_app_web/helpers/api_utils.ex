defmodule WandererAppWeb.Helpers.APIUtils do
  ...
  @spec respond_data(Plug.Conn.t(), any(), atom() | integer(), map()) :: Plug.Conn.t()
  def respond_data(conn, data, status \\ :ok, meta \\ %{}) do
    body =
      if use_legacy_format?(conn) do
        # Legacy format: return data directly or with simple wrapper
        case data do
          # Already wrapped
          %{data: _} -> data
          _ -> %{data: data}
  ...
  def error_response(conn, status, message, details \\ nil) do
    body =
      if use_legacy_format?(conn) do
        # Legacy format: {error: "message", details: "details"}
        if details, do: %{error: message, details: details}, else: %{error: message}
      else
        # New standard format: {errors: [{message: "message", details: "details"}]}
        error_item =
          if details do
  ...
  # Helper to determine response format based on request path
  defp use_legacy_format?(conn) do
    case conn.request_path do
      # v1 API uses new format
      "/api/v1" <> _ -> false
      # Legacy API uses old format
      "/api/" <> _ -> true
      # Default to legacy for non-API endpoints
      _ -> true
    end
  end
