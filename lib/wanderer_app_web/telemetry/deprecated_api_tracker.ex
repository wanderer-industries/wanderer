defmodule WandererAppWeb.Telemetry.DeprecatedApiTracker do
  ...
  def track_legacy_request(conn) do
    ...
    # Log deprecation warning in all environments except test (to reduce noise in tests)
    if Application.get_env(:wanderer_app, :env) != :test do
      Logger.warning("""
  ...
  # Helper function to safely convert remote IP to string
  defp safe_remote_ip(nil), do: "unknown"
  defp safe_remote_ip(remote_ip) do
    try do
