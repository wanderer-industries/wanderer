defmodule WandererAppWeb.Validations.ApiValidations do
  ...
  def validate_days_param(params) do
    ...
    {defaults, types}
    |> cast(params, [:days])
    |> validate_number(:days, greater_than: 0, less_than_or_equal_to: 30)
    |> apply_action(:validate)
  end
  ...
  defp validate_entity_role_restrictions(changeset) do
    ...
    cond do
      corp_id -> validate_admin_manager_role_restriction(changeset, role, "Corporation")
      alliance_id -> validate_admin_manager_role_restriction(changeset, role, "Alliance")
      true -> changeset
    end
  end
  ...
  defp validate_role_restrictions(changeset) do
    ...
    if entity_type in ["corporation", "alliance"] do
      validate_admin_manager_role_restriction(changeset, role, String.capitalize(entity_type))
    else
      changeset
    end
  end

  # Common helper for validating admin/manager role restrictions
  defp validate_admin_manager_role_restriction(changeset, role, entity_type) do
    if role in ["admin", "manager"] do
      add_error(changeset, :role, "#{entity_type} members cannot have #{role} role")
    else
      changeset
    end
  end
