defmodule WandererAppWeb.MapEventsChannel do
  ...
  defp handle_successful_join(map_id, map, payload, socket) do
    ...
    {:ok, %{status: "connected", map_id: map_id, map_name: map.name, event_filter: event_filter}, socket}
  end
  ...
  def terminate(reason, socket) do
    ...
    Logger.debug(fn -> "Client disconnected from external events for map #{map_id}, reason: #{inspect(reason)}" end)
    :ok
  end
  ...
  defp parse_event_filter(%{"events" => events}) when is_list(events) do
    ...
    |> Enum.filter(& &1)  # Remove nil values from invalid event types
    |> case do
      [] -> :all  # If no valid events specified, default to all
  ...
  defp parse_event_filter(%{"events" => ["*"]}), do: :all
  defp parse_event_filter(_), do: :all  # Default to all events if no filter specified
  ...
  defp should_send_event?(_event, _filter), do: true  # Default to sending if filter format is unexpected
end