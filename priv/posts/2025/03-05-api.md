...
- **Description:** Retrieves character activity data for a map, including passages, connections, and signatures, filtered by the specified time period.
- **Authentication:** Requires Map API Token.
- **Parameters:**
  - `map_id` (optional if `slug` is provided) — the UUID of the map.
  - `slug` (optional if `map_id` is provided) — the slug identifier of the map.
  - `days` (optional) — number of days to look back for activity data (defaults to 1 if not provided). All activity data (passages, connections, and signatures) will be filtered to only include records within this time period.
