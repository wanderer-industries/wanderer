...
defmodule OpenApiDiff do
  ...
  defp check_responses(path, old_responses, new_responses) do
    ...
    changes = changes ++ Enum.flat_map(old_responses, fn {code, old_resp} ->
      case Map.get(new_responses, code) do
        nil ->
          [%{type: :response_removed, path: path,
             description: "Response code #{code} removed"}]
        new_resp ->
          # Deep check response schema changes
          check_response_schema_changes(old_resp, new_resp, path, code)
      end
    end)
    ...
    changes = changes ++ Enum.flat_map(old_props, fn {prop_name, _old_prop} ->
      case Map.get(new_props, prop_name) do
        ...
        new_prop ->
          ...
          check_property_type_changes(old_prop, new_prop, "#{path}.#{prop_name}")
      end
    end)
  ...
  # Deep check response schema changes
  defp check_response_schema_changes(old_resp, new_resp, path, code) do
    changes = []
    
    # Check content type changes
    old_content = old_resp["content"] || %{}
    new_content = new_resp["content"] || %{}
    
    changes = changes ++ Enum.flat_map(old_content, fn {content_type, old_content_spec} ->
      case Map.get(new_content, content_type) do
        nil ->
          [%{type: :content_type_removed, path: "#{path}[#{code}]",
             description: "Content type '#{content_type}' removed from response"}]
        new_content_spec ->
          check_content_schema_changes(old_content_spec, new_content_spec, "#{path}[#{code}].#{content_type}")
      end
    end)
  ...
  # Check content schema changes  
  defp check_content_schema_changes(old_content, new_content, path) do
    old_schema = get_in(old_content, ["schema"])
    new_schema = get_in(new_content, ["schema"])
    
    if old_schema && new_schema do
      check_schema_changes(path, old_schema, new_schema)
    else
      []
    end
  end
  ...
  # Check property type changes
  defp check_property_type_changes(old_prop, new_prop, path) do
    changes = []
    ...
    changes = if old_type && new_type && old_type != new_type do
      changes ++ [%{type: :property_type_changed, path: path,
                    description: "Property type changed from '#{old_type}' to '#{new_type}'"}]
    else
      changes
    end
    
    # Check format changes
    old_format = old_prop["format"]
    new_format = new_prop["format"]
    
    changes = if old_format && new_format && old_format != new_format do
      changes ++ [%{type: :property_format_changed, path: path,
                    description: "Property format changed from '#{old_format}' to '#{new_format}'"}]
    else
      changes
    end
    
    # Check if property became nullable or non-nullable
    old_nullable = old_prop["nullable"] || false
    new_nullable = new_prop["nullable"] || false
    
    changes = cond do
      !old_nullable && new_nullable ->
        changes ++ [%{type: :property_became_nullable, path: path,
                      description: "Property became nullable"}]
      old_nullable && !new_nullable ->
        changes ++ [%{type: :property_became_non_nullable, path: path,
                      description: "Property became non-nullable (breaking change)"}]
      true ->
        changes
    end
